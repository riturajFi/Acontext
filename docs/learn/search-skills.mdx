...
to find relevant content and optionally generate synthesized answers from your knowledge spaces.
...
# Note: final_answer is typically None in fast mode
```
...
// Note: final_answer is typically null in fast mode
```
</CodeGroup>
...
Quick semantic search using embeddings. Returns blocks only, no answers.
</Card>

<Card title="Agentic Mode" icon="brain" iconType="duotone">
Agentic iterative search with refinement. Can generate synthesized answers.
</Card>
</CardGroup>

### Fast Mode Details
- **Speed**: ~100-500ms response time
- **Output**: Relevant blocks only (`final_answer` is null)
...
- **Output**: Blocks + optional synthesized answer
...
```json
{
  ...
  "final_answer": null  // null in fast mode, string in agentic mode
}
```
...
<Note>
In agentic mode, you can use the `final_answer` directly instead of building your own prompt, as the AI has already synthesized the information.
</Note>
